...
// Package config loads and validates the decision-service configuration.
//
// The service has three external surfaces:
//
//   - Postgres (the routing.routing_decisions table, read + idempotent insert).
//   - Streaming bus (routing.decision.v1 subscriber).
//   - HTTP API (read queries for the admin console).
package config
...
type ServerConfig struct {
	// Port is the TCP port the HTTP server binds to. Default 8094.
	Port int `yaml:"port"`
	// MaxPageSize caps the rows returned by a single /v1/decisions call.
	MaxPageSize int `yaml:"max_page_size"`
	// DefaultPageSize is the page size when the caller omits ?limit=.
	DefaultPageSize int `yaml:"default_page_size"`
}

// DatabaseConfig holds the decision-ledger Postgres connection.
type DatabaseConfig struct {
	// DSNEnv is the environment variable name holding the Postgres DSN.
	// Default OPENLLM_DECISION_DSN.
	DSNEnv string `yaml:"dsn_env"`
}

// BusConfig configures the streaming-bus consumer for routing.decision.v1.
type BusConfig struct {
	...
	// Topic is the bus topic carrying decision events.
	// Default routing.decision.v1.
	Topic string `yaml:"topic"`
}

// TopicDecisionEvent is the bus topic carrying routing-decision events.
const TopicDecisionEvent = "routing.decision.v1"
...
func (c *Config) applyDefaults() {
	...
	if c.Database.DSNEnv == "" {
		c.Database.DSNEnv = "OPENLLM_DECISION_DSN"
	}
	if c.Bus.ClientID == "" {
		c.Bus.ClientID = "openllm-decision-service"
	}
	if c.Bus.Group == "" {
		c.Bus.Group = "openllm-decision-service"
	}
	if c.Bus.Topic == "" {
		c.Bus.Topic = TopicDecisionEvent
	}
}
